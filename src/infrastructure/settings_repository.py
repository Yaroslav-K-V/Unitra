...
class EnvSettingsRepository:
    ...
    def save(self, api_key: str = "", model: str = "", show_hints=None, ai_policy: Optional[AiPolicy] = None) -> Dict[str, object]:
        ...
        if model or show_hints is not None or ai_policy is not None:
    ...
    def load(self) -> Dict[str, object]:
        env = self._load_env()
        settings = self._load_settings()
        model = str(settings.get("model") or env.get("OPENAI_MODEL") or self._default_model)
